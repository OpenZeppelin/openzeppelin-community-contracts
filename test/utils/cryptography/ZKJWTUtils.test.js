...
describe('ZKJWTUtils', function () {
  ...
  it('should validate ZKJWT sign hash', async function () {
    ...
    const fnSig = '$isValidZKJWT((string,bytes32,uint256,string,bytes32,bytes32,bool,bytes),address,address,bytes32)';
    await expect(this.mock[fnSig](jwtProof, this.jwtRegistry.target, this.verifier.target, hash)).to.eventually.equal(
      JWTProofError.NoError,
    );
  });

  it('should validate ZKJWT with template', async function () {
    const hash = ethers.hexlify(ethers.randomBytes(32));
  ...
  it('should detect invalid JWT proof', async function () {
    const hash = ethers.hexlify(ethers.randomBytes(32));
    const command = SIGN_HASH_COMMAND + ' ' + ethers.toBigInt(hash).toString();
    const jwtProof = buildJWTProof(command);
    ...
    const fnSig = '$isValidZKJWT((string,bytes32,uint256,string,bytes32,bytes32,bool,bytes),address,address,bytes32)';
    await expect(this.mock[fnSig](jwtProof, this.jwtRegistry.target, this.verifier.target, hash)).to.eventually.equal(
      JWTProofError.JWTProof,
    );
  });

  it('should validate proper kid|iss|azp domain format', async function () {
    const domain = 'auth0-key|https://your-domain.auth0.com/|your-auth0-client-id';
