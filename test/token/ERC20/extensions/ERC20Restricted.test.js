...
describe('ERC20Restricted', function () {
  ...
  describe('restriction management', function () {
    ...
    it('allows users with DEFAULT restriction', async function () {
      await expect(this.token.canSend(this.holder)).to.eventually.equal(true);
      await expect(this.token.canReceive(this.holder)).to.eventually.equal(true);
    });
    ...
    it('blocks users with BLOCKED status', async function () {
      ...
      await expect(this.token.canSend(this.holder)).to.eventually.equal(false);
      await expect(this.token.canReceive(this.holder)).to.eventually.equal(false);
    });
