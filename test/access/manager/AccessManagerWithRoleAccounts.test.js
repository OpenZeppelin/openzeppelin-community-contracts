...
describe('AccessManagerWithRoleAccounts', function () {
  ...
  describe('role account deployment', function () {
    it('deploys the role account at the predicted deterministic address', async function () {
      expect(await ethers.provider.getCode(this.predicted)).to.not.equal('0x');
    });

    it('getRoleAccount matches the address returned by deployRoleAccount', async function () {
      const otherRole = 7n;
      expect(await this.manager.deployRoleAccount.staticCall(otherRole)).to.equal(
  ...
  describe('ERC-1271 / ERC-7739 signature validation', function () {
    ...
    it('rejects a member signature with a corrupted inner signature', async function () {
      const domain = await getDomain(this.account);
      const signer = new ERC7739Signer(new RoleMemberSigner(this.member), domain);

      const text = 'authorize me';
      const hash = ethers.hashMessage(text);
      const signature = ethers.getBytes(await signer.signMessage(text));
      // Flip a byte inside the inner ECDSA signature (past the 20-byte address prefix).
      signature[25] ^= 0xff;

      expect(await this.account.isValidSignature(hash, ethers.hexlify(signature))).to.not.equal(ERC1271_MAGIC_VALUE);
    });
  });
