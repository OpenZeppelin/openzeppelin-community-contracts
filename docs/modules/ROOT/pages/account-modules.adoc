...
NOTE: The delayed executor's signature validation doesn't require a nonce since operations are uniquely identified by their operation ID (derived from account, salt, mode, and execution calldata) and cannot be scheduled twice.
