...
[NOTE]
====
For simplicity, `signers` should match in order with `signatures`. And also, to avoid duplicate usage of a single signature, the contract provides an xref:api:utils.adoc#MultiSignerERC7913-signerId-bytes-[`signerId`] function that exposes a single id for each signer. When providing a signature, the `signers` array should be in ascending order by `signerId`.
====
