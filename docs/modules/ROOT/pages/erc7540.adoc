...
contract NAVVault is ERC7540EpochDeposit, ERC7540EpochRedeem, Ownable {
    ...
    function _requestQueueLimit()
        internal view override(ERC7540EpochDeposit, ERC7540EpochRedeem) returns (uint256)
    {
...
For example, an oracle-driven strategy could mark requests claimable when an external price feed crosses a threshold; or a per-request strategy could give each request its own settlement timeline driven by off-chain compliance checks. (For the batched-epoch pattern, see the <<epoch-strategy>> section above.)
