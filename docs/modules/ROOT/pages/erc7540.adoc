...
By default `currentDepositEpoch()` and `currentRedeemEpoch()` return `(block.timestamp + 3 days) / 1 weeks`, so epochs transition at Monday 00:00 UTC (weeks start Monday). `block.timestamp` on any live chain is already many weeks past the Unix genesis, so this value is always > 0 — the epoch strategy is never mistaken for ERC-7540's controller-only (`requestId == 0`) accounting mode. Both functions are `virtual` and intended to be overridden in production deployments — common patterns are a manually-bumped counter incremented inside the admin fulfillment, a different cadence (daily, hourly), or a cyclic scheme tied to an external rebalance trigger.
...
NOTE: To enforce the per-controller isolation, the epoch strategy is a strict subset of ERC-7540 request auth: on top of the base's owner-side check, `requestDeposit` and `requestRedeem` also require `msg.sender` to be `controller` or an approved operator of `controller`. Third-party attribution (`msg.sender != controller`) — e.g. gifting a deposit to another account, or a router relaying a redeem via ERC-20 allowance — needs the controller's prior `setOperator` approval.

Users that hit the queue limit must claim fulfilled epochs to free up slots before submitting new requests. Because claims walk from the front, an unresolved oldest epoch blocks all later-fulfilled ones behind it and prevents queue cleanup — admins should resolve epochs close to FIFO order.
