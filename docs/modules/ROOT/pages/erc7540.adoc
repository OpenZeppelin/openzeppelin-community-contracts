...
ERC-7540 adds only two new entry points: `requestDeposit` and `requestRedeem`. Everything else builds on top of the existing ERC-4626 interface.
...
....
   ...
   └─────────┘                 └─────────┘             └───────────┘                  └─────────┘
                                Assets/shares           Exchange rate                  Shares/assets
                                locked in vault         is locked                      delivered to
                                                                                       receiver
....

* **Pending**: Assets (for deposits) or shares (for redeems) are locked in the vault. The exchange rate is not yet known.
* **Claimable**: The request has been fulfilled. The exchange rate is now locked. The controller can claim at any time.
...
| `ERC7540AdminDeposit`
| `ERC7540SyncRedeem`
| KYC-gated deposits, instant redemptions
