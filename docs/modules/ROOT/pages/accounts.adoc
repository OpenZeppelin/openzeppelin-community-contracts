...
OpenZeppelin provides an abstract xref:api:account.adoc#Account[`Account`] contract that implements the basic logic to handle user operations in compliance with ERC-4337. Developers who want to build their own account can use it to bootstrap custom implementations.
...
* xref:api:utils.adoc#SignerERC7913[`MultiSignerERC7913Weighted`]: Overrides the threshold mechanism of xref:api:utils.adoc#SignerERC7913[`MultiSignerERC7913`] with a weighted one.

TIP: Given xref:api:utils.adoc#SignerERC7913[`SignerERC7913`] provides a generalized standard for signature validation, you don't need to implement your own xref:api:utils.adoc#AbstractSigner[`AbstractSigner`]. Consider bringing your own ERC-7913 verifier.
