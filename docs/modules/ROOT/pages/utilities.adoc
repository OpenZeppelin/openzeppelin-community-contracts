...
_For prior knowledge on how to validate signatures on-chain, check out the xref:contracts::utilities.adoc#checking-signatures-on-chain[OpenZeppelin Contracts documentation]_

As opposed to validating plain-text messages, it is possible to let your users sign structured data (i.e. typed values) in a way that is still readable on their wallets. This is possible by implementing xref:contracts::api:utils/cryptography.adoc#EIP712[`EIP712`], a standard way to encode structured data into a typed data hash.

To start validating signed typed structures, just validate the xref:contracts::api:utils/cryptography.adoc#EIP712-_hashTypedDataV4-bytes32-[typed data hash]:
...
In case your smart contract validates signatures, using xref:contracts::api:utils/cryptography.adoc#ERC7739[`ERC7739`] signer will implement the xref:contracts::api:interfaces.adoc#IERC1271[`IERC1271`] interface for validating smart contract signatures following the approach suggested by ERC-7739:
...
* If `signer.length == 20`: verification is done using xref:contracts::api:utils/cryptography.adoc#SignatureChecker[SignatureChecker]
