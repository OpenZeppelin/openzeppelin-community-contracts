...
library EnumerableSetExtended {
     ...
     */
    function removeAt(Bytes32x2Set storage self, uint256 index) internal returns (bytes32[2] memory value) {
     ...
     * - `index` must be strictly less than {length}.
     */
    function _removeValueAt(Bytes32x2Set storage self, bytes32[2] memory value, uint256 index) private {
     ...
     * uncallable if the set grows to a point where copying to memory consumes too much gas to fit in a block.
     */
    function values(Bytes32x2Set storage self, uint256 start, uint256 end) internal view returns (bytes32[2][] memory) {
        unchecked {
