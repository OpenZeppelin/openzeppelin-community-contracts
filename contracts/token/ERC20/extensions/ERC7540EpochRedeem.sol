...
abstract contract ERC7540EpocRedeem is ERC7540 {
    ...
    function _sharesToFullfill(uint256 epochId) internal view virtual returns (uint256) {
        return epochId < currentEpoch() && _epochs[epochId].totalAssets == 0 ? _epochs[epochId].totalShares : 0;
    }

    /// Note: when epoch transition is manual, caller should bump the epoch before calling _fulfill
    function _fulfill(uint256 epochId, uint256 totalAssets) internal virtual {
        require(epochId < currentEpoch()); // TODO: too early
    ...
    function _computeWithdraw(uint256 assets, address controller) internal virtual override returns (uint256) {
        uint256 shares = 0;
    ...
    function _computeRedeem(uint256 shares, address controller) internal virtual override returns (uint256) {
        uint256 assets = 0;
