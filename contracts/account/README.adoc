 ...
 * {RoleAccount}: An account bound to a role of an `AccessManager` (through {RoleSigner}), acting on behalf of the current members of that role for both ERC-1271 signatures and ERC-7821 batched execution.
 * {RoleAccountFactory}: A factory that exposes a deterministic {RoleAccount} for each role of an external `AccessManager`.
